...
const Home = () => {
  ...
  // Sell List
  var resultSell = data.map((f) => {
    if (f.homePropertyType.toLowerCase() === "sell") {
      return (
        <>
          <div className="col-4">
  ...
  resultSell = resultSell.filter((item) => item !== undefined);

  //   Rent List
  var resultRent = data.map((f) => {
  ...
  resultRent = resultRent.filter((item) => item !== undefined);
  //   Buy List
  var resultBuy = data.map((f) => {
  ...
  resultBuy = resultBuy.filter((item) => item !== undefined);

import { Link } from "react-router-dom";
import "./agent.css";
import axios from "axios";
import { useEffect, useState } from "react";

const Agent = () => {
  const [data, setData] = useState([]);
  const [searchText, setSearchText] = useState("");

  useEffect(() => {
    getData();
  }, []);

  const getData = async () => {
    await axios
      .get("/users")
      .then((result) => setData(result.data))
      .catch((err) => console.log(err));
  };

  const filteredAgents = data.filter((agent) => {
    const fullName = `${agent.firstName} ${agent.lastName}`.toLowerCase();
    return fullName.includes(searchText.toLowerCase()) && agent.isbroker === true;
  });

  return (
    <>
      <div
        className="container"
        style={{
          marginLeft: "110px",
          backgroundColor: "#f7f7f7",
          width: "1280px",
        }}
      >
        <div className="row title">Agents</div>
        <div className="row searchinputagent">
          <div className="col">
            <input
              type="text"
              placeholder="Enter Agent Name"
              className="agentinput form-control"
              value={searchText}
              onChange={(e) => setSearchText(e.target.value)}
            />
          </div>
        </div>
        <div className="row agentbody">
          {filteredAgents.map((agent) => (
            <div className="col-3" key={agent._id}>
              <div className="card" style={{ width: "260px" }}>
                <Link to={`/agentdetail/${agent._id}`}>
                  <img
                  src={
                    agent.profilePhoto !== undefined
                      ? require(`D:/DU-DIET/Project/Codes/Back End/uploads/${agent.profilePhoto}`)
                      :`data:image/png;base64,`

                  }
                    className="card-img-top agentimg"
                    alt="..."
                  />
                </Link>
                <div className="card-body">
                  <Link to={`/agentdetail/${agent._id}`} className="agentname">
                    <h5>{agent.firstName} {agent.lastName}</h5>
                  </Link>
                  <h5 style={{ fontWeight: "normal" }}>Broker</h5>
                </div>
              </div>
            </div>
          ))}
        </div>
      </div>
    </>
  );
};

export default Agent;
